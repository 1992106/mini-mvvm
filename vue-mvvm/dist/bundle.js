...
function createElement(tagName, props) {
  ...
  vNode.events = Object.keys(props).filter(value => value.startsWith('on')).map(value => {
    return {
      propName: value,
      propValue: props[value]
    };
  });
  vNode.props = Object.keys(props).filter(value => !value.startsWith('on') && value !== 'key').map(value => {
    return {
      propName: value,
      propValue: props[value]
    };
...
function Vue(params) {
  ...
  const vNodeTree = params.render.call(_objectSpread(_objectSpread({}, params.data()), params.method), createElement);
  params.created && params.created();
  params.beforeMount && params.beforeMount();
  const el = document.querySelector(params.el); // TODO: diff
...
new Vue({
  ...
  render(h) {
    return h("div", null, h("span", null, "hello world"), h("button", {
      onclick: () => {
